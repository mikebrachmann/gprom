...
QueryOperator *
translateQuerySqlite (Node *node)
{
...
translateGeneral (Node *node)
{
        ...
        FOREACH(Node,stmt,(List *) result)
        {
            if (isA(stmt, ProvenanceStmt))
            {
                ...
                if(prov->summaryType == NULL)
                    stmt_his_cell->data.ptr_value = (Node *) translateQuerySqlite(stmt);
                else
                {
                    summaryType = prov->summaryType;
                    r = translateQuerySqlite(stmt);
    ...
    else
    {
        if (isA(node, ProvenanceStmt))
        {
            ...
            if(prov->summaryType == NULL)
                result = (Node *) translateQuerySqlite(node);
            else
            {
                summaryType = prov->summaryType;
                r = translateQuerySqlite(node);
                r->properties = copyObject(prop);
                result = (Node *) r;
            }
        }
    ...
    Set *done = PSET();
    disambiguiteAttrNamesSqlite(result, done);
...
boolean
disambiguiteAttrNamesSqlite(Node *node, Set *done)
{
    QueryOperator *op;
    ...
    if (isA(node, List))
    {
        ...
        {
            changed |= disambiguiteAttrNamesSqlite((Node *) q, done);
        }
        return changed;
    }
    ...
    {
        changed |= disambiguiteAttrNamesSqlite(child, done);
    }
...
translateSetQuery(SetQuery *sq)
{
    ...
    if (sq->lChild)
        left = translateQuerySqlite(sq->lChild);
    if (sq->rChild)
        right = translateQuerySqlite(sq->rChild);
    ASSERT(left && right);
...
//                if (updateCond != NULL)
                ...
                child = translateQuerySqlite(node);
        ...
        {
            child = translateQuerySqlite(prov->query);
            addChildOperator((QueryOperator *) result, child);
        }
        break;
...
{
    return translateQuerySqlite(fsq->subquery);
